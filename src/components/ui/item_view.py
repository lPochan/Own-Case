...
class ItemView:
    def __init__(self, lines, item, player, dialogue_box_sprite="text_box.png"):
    ...
    def next_line(self):
        ...
        if line[0] == '-':
            ...
            self.item_speak(line)


    def item_speak(self, line):
        self.speaker_label.set_text(self.item.obj_name)
    ...
    def command(self, line):
        ...
        if command == "give":
            ...
            if amount_added == 0:
                self.speaker_label.set_text("")
                self.content_label.set_text(f"")
            else:
                self.speaker_label.set_text("")
                self.content_label.set_text(f"Voce recebeu {amount_added} {t.name}")
        elif command == "goto":
